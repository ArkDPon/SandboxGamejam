        ...
        private class Pass : ScriptableRenderPass
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                foreach (var inst in _renderers)
                {
                    ...
                    // Iterate through each material on the renderer
                    if(inst.GetComponent<OutlineFx>().featuredMaterials.Length != 0) //render outline for specified mats
                    {
                        for (int i = 0; i < inst.GetComponent<OutlineFx>().featuredMaterials.Length; i++) 
                        {
                            if (inst._renderer.sharedMaterials[inst.GetComponent<OutlineFx>().featuredMaterials[i]] == null)
                            {
                                //Debug.LogWarning($"Material at index {i} on renderer {inst._renderer.gameObject.name} is null. Skipping.");
                                continue; // Skip null materials
                            }

                            cmd.SetGlobalTexture(s_MainTex, inst._renderer.sharedMaterials[inst.GetComponent<OutlineFx>().featuredMaterials[i]].mainTexture);
                            cmd.SetGlobalColor(s_Color, inst.Color);
                            cmd.DrawRenderer(inst._renderer, _owner._outlineMat, inst.GetComponent<OutlineFx>().featuredMaterials[i], 0); //Important!
                        }
                    }
                    else //render outline for all mats
                    {
                        for (int i = 0; i < inst._renderer.sharedMaterials.Length; i++)
                        {
                            if (inst._renderer.sharedMaterials[i] == null)
                            {
                                //Debug.LogWarning($"Material at index {i} on renderer {inst._renderer.gameObject.name} is null. Skipping.");
                                continue; // Skip null materials
                            }
                            ...
                            cmd.SetGlobalColor(s_Color, inst.Color);
                            cmd.DrawRenderer(inst._renderer, _owner._outlineMat, i, 0); //Important!
                        }
                    }
                }
                _renderers.Clear();
